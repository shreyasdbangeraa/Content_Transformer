...
- Lucide icons
- React Query/TanStack Query for server state where useful
...
Build an abstraction layer so the provider can be switched.

Preferred supported providers:
- Gemini
- OpenAI
- Other permitted LLM APIs

Do not hard-code provider-specific logic throughout the app.
...
Production:
- S3-compatible object storage or Cloudinary
...
Speech-to-text:
- provider/model appropriate to deployment and licensing
...
AI generates structured slide JSON -> code renders PPTX.

## Image generation

Use a permitted image generation model/API.

The architecture must keep image generation behind an abstraction.
...
- FFmpeg
- TTS
- generated/static visual assets
- subtitles
...
If the MVP can run synchronously for small documents, keep queue support modular and introduce background jobs for heavy tasks.
...
Select one based on deployment simplicity.
...
POST   /api/outputs/{id}/edit
POST   /api/outputs/{id}/fact-check
GET    /api/outputs/{id}/quality
...
Use background jobs for long-running transformations.
...
Use n8n as an external automation layer.
...
Do not send unapproved content.
...
Development should be reproducible with a simple command.
...
8. Implement PDF upload.
9. Implement PDF extraction.
10. Implement AI provider abstraction.
11. Implement canonical analysis.
12. Implement Executive Summary.
13. Implement LinkedIn.
14. Implement Advisory.
15. Implement X/Twitter.
16. Implement Presentation.
17. Implement multi-output generation.
18. Implement source references.
19. Implement fact checking.
20. Implement quality score.
21. Implement sensitive data detection.
22. Implement editor.
23. Implement approval.
24. Implement export.
25. Implement n8n integration.
26. Add advanced features only after the above is stable.

The first successful milestone should be:

> **Upload a fictional report -> analyze it -> select LinkedIn + Executive Summary + Advisory + Presentation -> generate all four -> verify claims -> approve -> export.**

Once this works end-to-end, continue with the advanced features.
