...
export default function PublishingPage() {
  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
      case 'PUBLISHED':
        return (
          <span className="rounded-md bg-emerald-50 text-emerald-800 border border-emerald-200 text-xs font-black px-2.5 py-1 flex items-center gap-1.5 uppercase">
      ...
      case 'SCHEDULED':
        return (
          <span className="rounded-md bg-sky-50 text-sky-800 border border-sky-200 text-xs font-black px-2.5 py-1 flex items-center gap-1.5 uppercase">
      ...
      default:
        return (
          <span className="rounded-md bg-amber-50 text-amber-800 border border-amber-200 text-xs font-black px-2.5 py-1 flex items-center gap-1.5 uppercase">
  ...
  return (
    <div className="space-y-10 animate-fade-in pb-20 max-w-7xl mx-auto w-full">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-5 border-b border-slate-200 pb-6">
        ...
        <button
          onClick={loadJobs}
          className="flex items-center gap-2 rounded-2xl border border-slate-300 bg-white px-5 py-2.5 text-sm font-bold text-slate-700 hover:bg-slate-50 hover:border-slate-400 active:scale-95 transition-all shadow-xs self-start sm:self-center"
        >
      ...
      <div className="rounded-3xl border border-indigo-200 bg-gradient-to-r from-indigo-50/80 via-sky-50/60 to-white text-slate-900 p-7 sm:p-8 space-y-6 shadow-md">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-5">
          <div className="flex items-center gap-4">
            <div className="flex h-12 w-12 items-center justify-center rounded-2xl bg-indigo-100 border border-indigo-200 text-indigo-700 p-2.5 shadow-xs">
          ...
          <div className="flex items-center gap-2">
            <span className="rounded-2xl bg-emerald-50 border border-emerald-200 text-emerald-800 text-xs sm:text-sm font-bold px-4 py-2 flex items-center gap-2 shadow-2xs">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 pt-2 text-sm">
          <div className="rounded-2xl border border-slate-200 bg-white p-4 flex items-center gap-3.5 shadow-2xs hover:border-sky-300 transition-all">
            <div className="p-2.5 rounded-xl bg-sky-50 border border-sky-200 text-sky-600">
              <Linkedin className="h-5 w-5 shrink-0" />
            </div>
            <div>
              <div className="font-bold text-slate-900 text-xs sm:text-sm">LinkedIn Publisher Node</div>
              <div className="text-[11px] text-slate-500 font-medium">Auto-banner &amp; hook injection</div>
            </div>
          </div>
          <div className="rounded-2xl border border-slate-200 bg-white p-4 flex items-center gap-3.5 shadow-2xs hover:border-sky-300 transition-all">
            <div className="p-2.5 rounded-xl bg-sky-50 border border-sky-200 text-sky-500">
              <Twitter className="h-5 w-5 shrink-0" />
            </div>
            <div>
              <div className="font-bold text-slate-900 text-xs sm:text-sm">X / Twitter Thread Node</div>
              <div className="text-[11px] text-slate-500 font-medium">Sequential thread dispatcher</div>
            </div>
          </div>
          <div className="rounded-2xl border border-slate-200 bg-white p-4 flex items-center gap-3.5 shadow-2xs hover:border-rose-300 transition-all">
            <div className="p-2.5 rounded-xl bg-rose-50 border border-rose-200 text-rose-600">
              <Instagram className="h-5 w-5 shrink-0" />
            </div>
            <div>
              <div className="font-bold text-slate-900 text-xs sm:text-sm">Instagram Carousel Node</div>
              <div className="text-[11px] text-slate-500 font-medium">Visual deck staging</div>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="rounded-3xl border border-slate-200 bg-white p-6 sm:p-8 space-y-6 shadow-xs">
        <div className="flex items-center justify-between border-b border-slate-200 pb-4">
          ...
          <span className="text-xs text-indigo-700 bg-indigo-50 border border-indigo-200 px-3 py-1 rounded-full font-bold">
        ...
        {loading ? (
          ...
          <div className="space-y-3">
            {jobs.map((job) => (
              <div
                key={job.id}
                className="rounded-2xl border border-slate-200 bg-slate-50/70 p-5 space-y-3 hover:bg-white hover:border-indigo-300 hover:shadow-md transition-all shadow-xs"
              >
